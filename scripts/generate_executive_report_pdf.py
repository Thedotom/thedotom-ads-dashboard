...
def build():
 ...
 cover=Table([[P('2026년 1~7월<br/>광고·슬롯 대표 보고서','title')],[P('실제 매출·광고비·GA4 광고 유입 후 구매를 연결해 광고 중단 위험을 점검하고,<br/>유지·축소·중단 대상을 구분하는 하반기 실행 기준을 제시합니다.','sub')]],colWidths=[174*mm],rowHeights=[72*mm,52*mm]);cover.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),NAVY),('BACKGROUND',(0,1),(-1,1),colors.HexColor('#172554')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('LEFTPADDING',(0,0),(-1,-1),15*mm)]));story += [Spacer(1,28*mm),cover,Spacer(1,13*mm),P('보고 기간  2026.01.01 - 2026.07.31','h2'),P('슬롯 운영 일정  2026.05.22 - 2026.08.09'),PageBreak()]
 story += [P('1. 경영 요약','h1')]
 ...
 evidence=[('확인된 효과',f"7월 자사몰 GA4 구매 매출 {money(gj['purchaseRevenue'])} 중 검색광고 유입 후 구매 매출은 {money(gj['paidSearchRevenue'])}으로 {pct(gj['paidSearchRevenueShare'])}입니다."),('전면 중단 위험',f"검색광고 유입 {int(gj['paidSearchSessions']):,}세션과 구매 {int(gj['paidSearchTransactions']):,}건의 접점이 동시에 사라질 수 있습니다."),('권고 결론','광고 전면 중단보다 고효율 검색광고는 유지하고 저효율 키워드·캠페인부터 단계적으로 축소하는 편이 타당합니다.'),('검증 방법','7일 단위로 예산을 줄인 뒤 광고비 절감액과 실제 매출 감소액을 비교하고, 매출 손실이 더 크면 즉시 복원합니다.')]
 ...
 story += [styled_table([[P(a,'smallb'),P(b)] for a,b in limits],[36*mm,138*mm],False),Spacer(1,7*mm),P('대표 의사결정 문장','h2'),P('<b>광고를 모두 끄는 결정보다 성과가 낮은 영역만 먼저 끄고, 실제 매출 변화로 나머지 광고의 필요성을 검증하는 것이 현재 데이터에 맞는 결론입니다.</b>'),PageBreak()]
 story += [P('4. 슬롯 효율','h1'),P('실제 진행 기간 2026.05.22 - 2026.08.09'),Spacer(1,4*mm)]
 ...
 decisions=[('전체','전면 지속 보류','운영 전 대조값이 없어 매출 증가를 슬롯 효과로 확정할 수 없고 후반 구간 매출이 둔화했습니다.'),('수건단품','지속 검증','비용률 약 1.0%, 평균 3.6위로 가장 양호합니다. 종료 후 7일 대조를 조건으로 유지합니다.'),('세트모음전','축소 검증','비용률 약 3.8%, 평균 8.2위이며 최근 매출이 초기보다 낮아 구간과 예산을 줄입니다.'),('조구만고리수건','중단 대조','평균 2.7위지만 비용률 약 24.0%입니다. 중단 후 매출·순위 유지 여부를 비교합니다.')]
 story += [styled_table([[P(a,'smallb'),P(b,'smallb'),P(c)] for a,b,c in decisions],[30*mm,28*mm,116*mm],False),Spacer(1,6*mm),P('판단 한계','h2'),P('집계 매출은 광고와 프로모션이 함께 반영된 관찰값입니다. 현재 데이터만으로 슬롯이 매출을 늘렸다고 단정할 수 없으며, 다음 운영부터는 시작 전 10일·운영 10일·종료 후 7일을 같은 상품 기준으로 비교해야 합니다.'),PageBreak(),P('5. 현재 운영 상태와 하반기 실행안','h1')]
