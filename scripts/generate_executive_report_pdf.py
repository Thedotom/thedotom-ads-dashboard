...
def build():
 ...
 cover=Table([[P('2026년 1~7월<br/>광고·슬롯 대표 보고서','title')],[P('실제 매출과 광고·마케팅비를 슬롯비와 분리해 광고 효율을 재정리하고,<br/>5월 22일부터 시작된 슬롯 운영의 성과와 하반기 실행 기준을 함께 제시합니다.','sub')]],colWidths=[174*mm],rowHeights=[72*mm,52*mm]);cover.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,0),NAVY),('BACKGROUND',(0,1),(-1,1),colors.HexColor('#172554')),('VALIGN',(0,0),(-1,-1),'MIDDLE'),('LEFTPADDING',(0,0),(-1,-1),15*mm)]));story += [Spacer(1,28*mm),cover,Spacer(1,13*mm),P('보고 기간  2026.01.01 - 2026.07.31','h2'),P('슬롯 운영 일정  2026.05.22 - 2026.08.09'),PageBreak()]
 story += [P('1. 경영 요약','h1')]
 ...
 conclusions=[('비용 분리',f"광고·마케팅비 {money(marketing_cost)}에는 슬롯비 {money(report_slot_cost)}를 포함하지 않음"),('실제 효율 중심',f"슬롯 제외 누적 실제 MER {mult(report_mer)}, 마케팅비율 {pct(marketing_rate)}"),('슬롯 효과 판단','운영 전 대조값이 없어 매출 증가를 슬롯 효과로 확정할 수 없음'),('운영 권고','수건단품 지속 검증 · 세트모음전 축소 · 조구만고리수건 중단 대조')];story += [styled_table([[P(a,'smallb'),P(b)] for a,b in conclusions],[36*mm,138*mm],False),PageBreak()]
 ...
 story += [styled_table([[P(a,'smallb'),P(b,'smallb'),P(c)] for a,b,c in decisions],[30*mm,28*mm,116*mm],False),Spacer(1,6*mm),P('판단 한계','h2'),P('집계 매출은 광고와 프로모션이 함께 반영된 관찰값입니다. 현재 데이터만으로 슬롯이 매출을 늘렸다고 단정할 수 없으며, 다음 운영부터는 시작 전 10일·운영 10일·종료 후 7일을 같은 상품 기준으로 비교해야 합니다.'),PageBreak(),P('4. 현재 운영 상태와 하반기 실행안','h1')]
