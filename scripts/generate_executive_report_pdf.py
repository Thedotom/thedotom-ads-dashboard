...
def build():
 ...
 story += [styled_table(mapping_table,[42*mm,24*mm,34*mm,22*mm,27*mm,20*mm,27*mm,18*mm]),Spacer(1,4*mm),P('상품 연결 기준','h2'),P('쇼핑검색 광고그룹을 스마트스토어 상품 ID로 연결해 광고 성과와 7월 실제 상품매출을 함께 표시했습니다. 동일 상품에 여러 광고그룹이 연결된 경우 광고비는 합산하고 상품매출은 중복 집계하지 않았습니다.'),Spacer(1,7*mm),P('운영 결정','h2'),P(f"<b>쇼핑검색은 1~7월 네이버 귀속 ROAS {mult(sh['adRoas'])}를 근거로 전체 운영을 유지합니다. 연결상품 중 광고비 대비 실제 매출·주문 성과가 낮은 광고그룹만 감액 또는 OFF하고, 조정 전후 7일 성과를 비교해 재개 여부를 결정합니다.</b>"),PageBreak()]
