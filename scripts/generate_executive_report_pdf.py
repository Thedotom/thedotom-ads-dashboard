...
def build():
 ...
 story += [styled_table([[P(a,'smallb'),P(b)] for a,b in limits],[36*mm,138*mm],False),Spacer(1,7*mm),P('의사결정 문장','h2'),P('<b>광고를 모두 끄는 결정보다 성과가 낮은 영역만 먼저 끄고, 실제 매출 변화로 나머지 광고의 필요성을 검증하는 것이 현재 데이터에 맞는 결론입니다.</b>'),PageBreak()]
 ...
 decisions=[('전체','전면 지속 보류','운영 전 21일 상품매출 대조값은 확보했습니다. 첫 21일 일평균은 수건단품 -2%, 세트모음전 +7%, 조구만고리수건 -10%로 차이가 있어 일괄 연장 대신 상품별 차등 운영이 필요합니다.'),('수건단품','지속 검증','비용률 약 1.0%, 평균 3.6위로 가장 양호합니다. 종료 후 7일 대조를 조건으로 유지합니다.'),('세트모음전','축소 검증','비용률 약 3.8%, 평균 8.2위이며 최근 매출이 초기보다 낮아 구간과 예산을 줄입니다.'),('조구만고리수건','중단 대조','평균 2.7위지만 비용률 약 24.0%입니다. 중단 후 매출·순위 유지 여부를 비교합니다.')]
